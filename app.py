...
def compare_samples(ref_2darray, test_2darrays):
    ...
    test_1_association = Associations(sampleImage=Sample(test_2darrays[0]).convertToAngle(), usePlainDataframes=True, includeDistance=True, limitDistance=100)
    test_2_association = Associations(sampleImage=Sample(test_2darrays[1]).convertToAngle(), usePlainDataframes=True, includeDistance=True, limitDistance=100)
